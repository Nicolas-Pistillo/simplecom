...
class="animate__animated animate__bounceInLeft grid grid-cols-12 gap-x-4 gap-y-3 items-end">
    ...
    <fieldset class="no-select col-span-full rounded-lg overflow-hidden border shadow-sm" aria-label="Shipping Rates">
        ...
        @foreach ($payment_methods as $method)
            <div wire:key='{{ $method->id }}' class="-space-y-px transition-colors duration-300
    ...
    <div class="no-select col-span-full flex items-center gap-3">
        ...
        <x-button wire:loading.remove wire:target='confirmOrder' 
        wire:click='setStep(2)' type="soft" :disabled="false" size="big" class="mt-8 !shadow">
            Volver
        </x-button>

        <x-button wire:loading.remove wire:target='confirmOrder' wire:click='confirmOrder' size="big" class="mt-8" :disabled="!isset($selected_payment_method)">
            Confirmar
        </x-button>
